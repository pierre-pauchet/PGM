...
for _ in range(runs):
    ...
    S = np.array(~np.isnan(Xnan), dtype=np.float64)
    ...
    # ---- fit not-MIWAE-mask---- #
    # ---------------------- #
    notmiwae_mask = notMIWAE(Xnan, Xval, n_latent=dl, n_samples=n_samples, n_hidden=n_hidden, missing_process=mprocess,mask=True, name=name)

    # ---- do the training
    trainer.train(notmiwae_mask, batch_size=batch_size, max_iter=max_iter, name=name + 'notmiwae_mask')

    # ---- find imputation RMSE
    RMSE_notmiwae_mask.append(utils.not_imputationRMSE(notmiwae_mask, Xtrain, Xz, Xnan, S, L)[0])


    # ---------------------- #
    # ---- fit not-MIWAE-mask---- #
    # ---------------------- #
    notmiwae = notMIWAE(Xnan, Xval, n_latent=dl, n_samples=n_samples, n_hidden=n_hidden, missing_process=mprocess,mask=False, name=name)
    ...
    """
    print('RMSE, MIWAE {0:.5f}, notMIWAEmasl {1:.5f}, notMIWAE {2:.5f}, MEAN {3:.5f}, MICE {4:.5f}, missForest {5:.5f}'
          .format(RMSE_miwae[-1], RMSE_notmiwae_mask[-1],RMSE_notmiwae[-1], RMSE_mean[-1], RMSE_mice[-1],0))


print("RMSE_miwae = {0:.5f} +- {1:.5f}".format(np.mean(RMSE_miwae), np.std(RMSE_miwae)))
print("RMSE_notmiwae_mask = {0:.5f} +- {1:.5f}".format(np.mean(RMSE_notmiwae_mask), np.std(RMSE_notmiwae_mask)))
