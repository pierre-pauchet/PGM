...
class notMIWAE:
    ...
    def __init__(self, X, Xval,
        ...
        with tf.variable_scope('encoder'):
            ...
            self.q_mu_m, self.q_log_sig2_m = self.encoder(self.in_pl)
            

        # ---- variational distribution
        q_z = tfp.distributions.Normal(loc=self.q_mu, scale=tf.sqrt(tf.exp(self.q_log_sig2)))
        q_z_m = tfp.distributions.Normal(loc=self.q_mu_m, scale=tf.sqrt(tf.exp(self.q_log_sig2_m)))
        ...
        self.l_z_m = q_z_m.sample(self.n_pl)  # shape [n_samples, batch_size, d]
        self.l_z_m = tf.transpose(self.l_z_m, perm=[1, 0, 2])  # shape [batch_size, n_samples, d]
        ...
        if out_dist in ['gauss', 'normal', 'truncated_normal']:
            ...
            with tf.variable_scope('data_process'):
                ...
                self.mu_m, std_m = self.gauss_decoder(self.l_z_m)
        ...
        with tf.variable_scope('missing'):
            ...
            if self.mask : 
                self.logits_miss= self.bernoulli_decoder_miss_z_mask(self.l_out_mixed,self.mu_m)
            else : 
        ...
        self.p_s_given_x_z_mask = tfp.distributions.Bernoulli(logits=self.logits_miss)  # (probs=self.s + self.eps)

        # ---- evaluate s in p(s|x,z)
        self.log_p_s_given_x = tf.reduce_sum(self.p_s_given_x.log_prob(tf.expand_dims(self.s_pl, axis=1)), axis=-1)

        b = self.p_s_given_x_z_mask.log_prob(tf.compat.v1.expand_dims(self.s_pl, axis=1))
        self.log_p_s_given_x_z_mask = tf.compat.v1.reduce_sum(  b, axis=-1)  # sum over d-dimension
        ...
        self.log_q_z_mask_given_x = tf.compat.v1.reduce_sum(q_z2_m.log_prob(self.l_z_m), axis=-1)
        ...
        prior = tfp.distributions.Normal(loc=0.0, scale=1.0)
        self.log_p_z_mask = tf.compat.v1.reduce_sum(prior.log_prob(self.l_z_m), axis=-1)
        ...
        # ---- notMIWAE_mask:
        if self.mask : 
            self.notMIWAEmask = self.get_notMIWAE_mask(self.log_p_x_given_z,
                                          self.log_q_z_given_x,
                                          self.log_p_z,
                                          self.log_p_s_given_x_z_mask,
                                          self.log_p_z_mask,
                                          self.log_q_z_mask_given_x)
        else: 
    ...
    def bernoulli_decoder_miss_z_mask(self, l_out,mu_m):

        if self.missing_process == 'selfmasking':

            self.W_m = tf.get_variable('W', shape=[1, 1, self.d])
            self.b_m = tf.get_variable('b', shape=[1, 1, self.d])
            self.gamma = tf.get_variable('gamma', shape=[1, 1, self.d])


            logits = - self.W_m * (l_out - self.b_m) + self.gamma*mu_m

        elif self.missing_process == 'selfmasking_known':

            self.W_m = tf.get_variable('W', shape=[1, 1, self.d])
            self.W_m = tf.nn.softplus(self.W_m)
            self.b_m = tf.get_variable('b', shape=[1, 1, self.d])
            self.gamma = tf.get_variable('gamma', shape=[1, 1, self.d])
            ...
            concat = l_out + mu_m  # Shape (2, 4)

            logits = keras.layers.Dense(units=self.d, activation=None, name='y')(concat)

        elif self.missing_process == 'nonlinear':

            l_out = keras.layers.Dense(units=self.n_hidden, activation=tf.nn.tanh, name='y')(l_out)
            logits = keras.layers.Dense(units=self.d, activation=None, name='y')(l_out)
    ...
    def get_notMIWAE_mask(self, lpxz, lqzx, lpz,lpsxzm,lpzm,lqzmx):
        """" the not-MIWAE ELBO """
        ...
        # ---- average over samples
        log_avg_weight = log_sum_w - tf.compat.v1.log(tf.compat.v1.cast(self.n_pl, tf.compat.v1.float32))
